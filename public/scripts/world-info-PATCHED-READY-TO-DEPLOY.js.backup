...
// Lorebook filter cache with TTL
const lorebookFilterCache = new Map();
const CACHE_TTL = 5 * 60 * 1000; // 5 minutes

/**
 * Filters lorebooks based on user permissions
 * @param {string[]} worldNames - All available lorebook names
 * @returns {string[]} Filtered lorebook names the user can access
 */
function filterLorebooksForUser(worldNames) {
    ...
    const filtered = worldNames.filter(name => {
        // ADMIN- prefix: admin-only lorebooks
        if (name.startsWith('ADMIN-')) {
            return false;
        }

        // USER-{handle}- prefix: user-specific lorebooks
        const userMatch = name.match(/^USER-([^-]+)-/);
        if (userMatch) {
            return userMatch[1] === currentUserHandle;
        }

        // SHARED- prefix or no prefix: public lorebooks
        return true;
    });

    // Log denied books for security audit
...
/**
 * Clears the entire lorebook filter cache.
 */
...
export async function updateWorldInfoList() {
    ...
    if (result.ok) {
        ...
        // Visibility filter: restrict which world names are visible to non-admins
        world_names = filterLorebooksForUser(world_names);
        invalidateLorebookCache(); // Clear cache when list is refreshed
